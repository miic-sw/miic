...
def merge_corr_results(par):
    ...
    # combine matrices of subdivisions
    if ('subdivision' in par['co']) and not par['co']['subdivision']['recombine_subdivision']:
        for cind in comb_ind:
            ...
            comb_str = 'mat__%s%s.%s%s.*.%s%s.mat' % (ID0[0],ID1[0],ID0[1],ID1[1],ID0[3],ID1[3])
            matl = []
            ...
            for year in year_list:
                ...
                for day in day_list:
                    try:
                        fname = glob.glob(os.path.join(day,comb_str))[0]
                        # the following line avoids including traces after a change in the location code
                        comb_str = os.path.split(fname)[1]
                        mat = mat_to_ndarray(fname)
                        matl.append(mat)
                    except:
                        logger.warning("Problem with fname: %s" % 
                                        (comb_list[0][cind], comb_list[1][cind], sys.exc_info()[0]))
            try:
                mat = corr_mat_merge(matl)
                tr = corr_mat_extract_trace(mat,method='norm_mean')
                save_dict_to_matlab_file(os.path.join(par['co']['res_dir'],comb_str.replace('mat__','ctr__')),tr)
                rmat = corr_mat_resample(mat,sttimes)
                save_dict_to_matlab_file(os.path.join(par['co']['res_dir'],comb_str),rmat)
            except:
                logger.warning("Problem with combination %s-%s: %s" % 
                                (comb_list[0][cind], comb_list[1][cind], sys.exc_info()[0]))

    # No subdivisions or subdivisions already recombined
    else:
        for cind in comb_ind:
            ID0 = comb_list[0][cind].split('.')
            ID1 = comb_list[1][cind].split('.')
            comb_str = 'tr__%s%s.%s%s.*.%s%s.mat' % (ID0[0],ID1[0],ID0[1],ID1[1],ID0[3],ID1[3])
            trl = []
            year_list = dir_read(par['co']['res_dir'],'????')
            year_list = sorted(year_list)
            for year in year_list:
            ...
            try:
                ...
                tr = corr_mat_extract_trace(mat,method='norm_mean')
                save_dict_to_matlab_file(os.path.join(par['co']['res_dir'],comb_str.replace('tr__','ctr__')),tr)
                rmat = corr_mat_resample(mat,sttimes)
                save_dict_to_matlab_file(os.path.join(par['co']['res_dir'],comb_str.replace('tr__','mat__')),rmat)
            except:
                logger.warning("Problem with combination %s-%s: %s" % 
                                (comb_list[0][cind], comb_list[1][cind], sys.exc_info()[0]))
